...
class HifuseModel(nn.Module):
    def __init__(self, num_classes, patch_size=4, in_chans=3, embed_dim=96, depths=(2, 2, 2, 2),
        ...
        self.norm_layer_0 = LayerNormChannelsFirst(96, eps=1e-6)

        # Stage 1
        self.norm_layer_1 = LayerNormChannelsFirst(96, eps=1e-6)
        self.local_bock_1 = LocalFeatureExtractor(channels=96, dropout_prob=0.0)

        # Stage 2 
        self.norm_layer_2 = LayerNormChannelsFirst(conv_dims[1], eps=1e-6)
        self.conv_layer_2 = nn.Conv2d(conv_dims[0], conv_dims[1], kernel_size=2, stride=2)
        self.local_bock_2 = LocalFeatureExtractor(channels=conv_dims[1], dropout_prob=0.0)
        
        # Stage 3 
        self.norm_layer_3 = LayerNormChannelsFirst(conv_dims[2], eps=1e-6)
        self.conv_layer_3 = nn.Conv2d(conv_dims[1], conv_dims[2], kernel_size=2, stride=2)
        self.local_bock_3 = LocalFeatureExtractor(channels=conv_dims[2], dropout_prob=0.0)
        
        # Stage 4 
        self.norm_layer_4 = LayerNormChannelsFirst(conv_dims[3], eps=1e-6)
        self.conv_layer_4 = nn.Conv2d(conv_dims[2], conv_dims[3], kernel_size=2, stride=2)
        self.local_bock_4 = LocalFeatureExtractor(channels=conv_dims[3], dropout_prob=0.0)

        # # --------------------- Novelty ---------------------------#
        # self.local_block_1 = LocalFeatureExtractor(in_channels=3, out_channels=96, dropout_prob=0.0)

        # self.local_block_2 = LocalFeatureExtractor(in_channels=96, out_channels=192, dropout_prob=0.0)

        # self.local_block_3 = LocalFeatureExtractor(in_channels=192, out_channels=384, dropout_prob=0.0)

        # self.local_block_4 = LocalFeatureExtractor(in_channels=384, out_channels=768, dropout_prob=0.0)
    ...
    def forward(self, imgs):
        ...
        x_c = self.norm_layer_0(x_c)
        x_c_1 = self.local_bock_1(x_c)

        # Stage 2

        x_c = self.conv_layer_2(x_c_1)
        x_c = self.norm_layer_2(x_c)
        x_c_2 = self.local_bock_2(x_c)

        # Stage 3

        x_c = self.conv_layer_3(x_c_2)
        x_c = self.norm_layer_3(x_c)
        x_c_3 = self.local_bock_3(x_c)

        # Stage 4

        x_c = self.conv_layer_4(x_c_3)
        x_c = self.norm_layer_4(x_c)
        x_c_4 = self.local_bock_4(x_c)        
...
def main():
    ...
    # Stage 1
    extractor = LocalFeatureExtractor(channels=96, dropout_prob=0.0)
    local_branch_1 = extractor(convolved_tensor_local)
