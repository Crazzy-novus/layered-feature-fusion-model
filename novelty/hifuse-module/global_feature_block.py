...
class ShiftedWindowAttention(nn.Module):
    """
    A module implementing multi-head self-attention (MSA) within a defined window, 
    including the option for shifted windows and relative positional encoding.
    
    Parameters:
        input_dim (int): The number of channels in the input.
        win_size (tuple[int]): Dimensions of the attention window (height, width).
        num_heads (int): Number of attention heads for the computation.
        include_bias (bool, optional): If True, adds a bias term to the query, key, and value projections. Default: True.
        attention_dropout (float, optional): Dropout applied to the attention weights. Default: 0.0.
        projection_dropout (float, optional): Dropout applied to the output projection. Default: 0.0.
    """
